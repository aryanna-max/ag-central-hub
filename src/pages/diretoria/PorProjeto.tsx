...
import {
  EXEC_STATUS_LABELS,
  EXEC_STATUS_CHART_COLORS as EXEC_STATUS_COLORS,
  BILLING_LABELS,
  DASHBOARD_GROUPS as GROUPS,
  DASHBOARD_ALL_COLUMNS as ALL_COLUMNS,
...
export default function PorProjeto() {
  ...
  const prazoCriticoCount = useMemo(() => {
    ...
    return projects.filter((p) => {
      ...
      if (!es || ["entregue", "faturamento", "pago"].includes(es)) return false;
...
function ProjectCard({
  ...
  const p = project;
  const billingLabel = p.billing_type ? BILLING_LABELS[p.billing_type] || p.billing_type : null;
